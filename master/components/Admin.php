 ...
 * Admin module. Common methods for admin side of site
 * @package		Project
 * @subpackage	Component
 * @author		amostovoy
 */
abstract class AdminComponent extends CommonController
{
    /**
...
//            $this->checkIsAdmin();
        ...
        {
            $this->checkIsAdmin(true, $this->_request->getControllerName());
        ...
        $this->_view->addJsVar(array('is_backend' => true));
    ...
    {
        $this->formSiteTitle($this->_lang->{$this->module_lang}()->site_title_admin);
    }

    /**
     * function to check admin logged in status
     * @param	bool	$is_redirect    (optional) (default:false) redirect or not after check failure
     * @param	string  $next           (optional) (default:null) next controller name to redirect after admin login
     * @return	bool					return false if user is not admin
     */
    protected function checkIsAdmin($is_redirect=false, $next=null)
    {
        if ( !$this->_user->isAdmin()) {
            $is_redirect and $this->redirect($this->base_url.(!empty($next)?'/auth/login/next/'.urlencode($next):''), 'top');
            return false;
        }
        return true;
    }
    ...
    public function indexAction()
    {
        ...
        $this->_view->addJsLangVar( array('are_you_sure' => $this->_lang->admin_table()->are_you_sure) );
    }
    ...
    public function manageAction()
    {
        ...
        if($this->backbutton)
        {
            if(isset($_SERVER['HTTP_REFERER']) 
                ...
                $_SESSION['admin_manage_backbtn'] = $_SERVER['HTTP_REFERER'];
            }
            if(isset($_SESSION['admin_manage_backbtn']))
            {
                $sf = '/'.Controller::SAVE_FILTER.'/1';
                $this->addToolbarActionButton(array(
                    'title' => $this->_lang->button()->back,
                    'href'  => str_replace($sf, '', $_SESSION['admin_manage_backbtn']).$sf
                ));
            }
        }
    ...
    protected function logout()
    {
		...
		$this->redirect($this->domain_url .'/'. Config::ADMIN_URL);
    }
}

/* End of file Admin.php */
/* Location: ./components/Admin.php */
?>